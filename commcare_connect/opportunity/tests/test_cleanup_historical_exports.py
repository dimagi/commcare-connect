...
# boto3 is only in production requirements, so we inject a mock module for tests
mock_boto3_module = mock.MagicMock()
sys.modules.setdefault("boto3", mock_boto3_module)
...
class TestExportFilenamePatterns:
    ...
    def test_matches_export_filenames(self, filename):
        assert any(re.match(p, filename) for p in EXPORT_FILENAME_PATTERNS)
    ...
    def test_does_not_match_non_export_filenames(self, filename):
        assert not any(re.match(p, filename) for p in EXPORT_FILENAME_PATTERNS)
...
class TestCleanupHistoricalExportsCommand:
    def test_dry_run_does_not_delete(self, settings):
        ...
        mock_boto3_module.reset_mock()
        mock_boto3_module.resource.return_value.Bucket.return_value = mock_bucket
    ...
    def test_deletes_matching_old_files(self, settings):
        settings.AWS_STORAGE_BUCKET_NAME = "test-bucket"
        old_date = datetime.now(timezone.utc) - timedelta(days=60)
