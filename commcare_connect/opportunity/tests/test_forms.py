...
from commcare_connect.program.utils import opportunity_manage_org_ids
...
class TestSupervisingOrganizations:
    def test_includes_program_organization(self, opportunity):
        assert opportunity.program.organization in supervising_organizations(opportunity.program)
    ...
    def test_includes_funder(self, opportunity, funder_org):
        ...
        assert funder_org in supervising_organizations(program)
    ...
    def test_includes_accepted_applicant(self, opportunity):
        ...
        assert applicant in supervising_organizations(opportunity.program)
    ...
    def test_excludes_applicant_without_accepted_status(self, opportunity, status):
        ...
        assert applicant not in supervising_organizations(opportunity.program)

    def test_excludes_unrelated_organization(self, opportunity):
        assert OrganizationFactory() not in supervising_organizations(opportunity.program)
    ...
    def test_program_without_funder_does_not_error(self, opportunity):
        ...
        assert program.organization in supervising_organizations(program)
    ...
    def test_results_are_distinct(self, opportunity):
        ...
        ids = [org.id for org in supervising_organizations(opportunity.program)]
...
class TestSupervisingOrganizationOnEdit(SupervisingOrganizationFormTestBase):
    ...
    def test_changing_supervisor_moves_oversight(self, opportunity):
        previous = OrganizationFactory()
        ProgramApplicationFactory(
            organization=previous, program=opportunity.program, status=ProgramApplicationStatus.ACCEPTED
        )
        opportunity.supervising_organization = previous
        opportunity.save()

        form = self._update_form(opportunity, supervising_organization=opportunity.organization.pk)
        ...
        assert opportunity.organization.id in opportunity_manage_org_ids(updated)
        assert previous.id not in opportunity_manage_org_ids(updated)
...
class TestSupervisingOrganizationOnChangeForm:
    ...
    def _request_for(self, org, role=UserOrganizationMembership.Role.ADMIN):
        user = UserFactory()
        return StubRequest(user=user, org=org, membership=make_membership(org, user, role))
