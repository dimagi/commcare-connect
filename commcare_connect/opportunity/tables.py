...
class UserVisitTable(OrgContextTable):
    ...
    def render_details(self, record):
        ...
        return mark_safe(
            f'<a class="inline-flex items-center px-2 py-1 rounded text-sm font-medium bg-blue-600 '
            f'text-white hover:bg-blue-700" href="{url}">Review</a>'
        )
...
class SuspendedUsersTable(tables.Table):
    ...
    def render_revoke_suspension(self, record, value):
        ...
        return format_html(
            '<a class="inline-flex items-center px-2 py-1 rounded text-sm font-medium bg-green-600'
            'text-white hover:bg-green-700" href="{}?next={}">Revoke</a>',
            revoke_url,
            page_url,
        )
