...
class InvoiceReportTable(tables.Table):
    ...
    exchange_rate = columns.Column(
        empty_values=(None,), accessor="exchange_rate__rate", verbose_name=_("Exchange Rate")
    )
    invoice_type = columns.Column(verbose_name=_("Type"), accessor="service_delivery")
    status = columns.Column(verbose_name=_("Status"))
    invoice_creation_date = columns.Column(verbose_name=_("Invoice Creation Date"), accessor="date")
    status_updated_date = DMYTColumn(verbose_name=_("Invoice Last Updated Date"), accessor="status_updated_date")
